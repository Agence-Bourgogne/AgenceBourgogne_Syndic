...
public partial class PublishDocument : Form
{
    ...
    private List<FilesList> listFiles;
    //string filename ="";
    private string _copro_id = "";
    private string _immeuble_id = "";
        ...
        if (_immeuble != null)
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow item in ListFilesView.Rows)
                {
                    ...
                    try
                    {
                        ServiceReferenceUtils.SendReportPDF(fileName, title, Guid.NewGuid().ToString(), _immeuble_id, _copro_id);
                        dialogResult = true;
                    }
                    catch
                    { }
    ...
    private void FillDataGridView(string[] files)
    {
        ...
        foreach (var filename in files)
        {
            if(!listFiles.Exists(x=>x.Fichier == filename))
            {
                ...
                {
                    Fichier = filename,
                    Titre = GetUniqueTitle(Path.GetFileNameWithoutExtension(filename) + "_" + DateTime.Now.ToString("dd-M-yyyy"))
                };
                listFiles.Add(file);
            }
        }
