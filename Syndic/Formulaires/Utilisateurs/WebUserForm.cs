...
public partial class WebUserForm : Form
{
    ...
    private UserEntitie _currentUsr;
    ...
    private void FillTreeView()
    {
        ...
        try
        {
            ...
            var usr = (UserEntitie)dataGridView.SelectedRows[0].DataBoundItem;
            _currentUsr = usr;
    ...
    private void btnAddDoc_Click(object sender, EventArgs e)
    {
        ...
        if (current != null && current.Tag != null)
        {
            if (current.Tag is ImmeubleEntite tag)
                immeuble = tag;
            else if (current.Tag is CoproprietaireEntite entite)
            {
                copro = entite;
                immeuble = copro.Immeuble;
            }
            var publishForm = new PublishDocument(immeuble, copro);
