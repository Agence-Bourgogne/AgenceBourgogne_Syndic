...
{
    private DataRowView currentLot;
    public ImmeubleEntite immeuble;
    private DataTable repartitionImmeuble; //= immeuble.getRepartitionImmeuble();
    private DataTable repartitionLots;
    ...
    private void FicheRepartLot_FormClosing(object sender, FormClosingEventArgs e)
    {
        ...
        foreach (DataGridViewRow r in dataGridViewLots.Rows)
        {
            ...
            if (lot.statut == (int)GlobalConstantes.StatutData.Supprime) listToDel.Add(lot);
        }

        if (listToDel.Count > 0)
        {
            var res = MessageBox.Show("Des lot ont été marqués comme supprimés définivement voulez les effacer ?",
            ...
            else if (res == DialogResult.Yes)
            {
                ...
                try
                {
                    foreach (var lot in listToDel) LotDescriptionController.getController().deleteEntite(lot);
                    trx.Commit();
    ...
    private void ShowFromRepartitionImmeuble()
    {
        ...
        foreach (DataRow row in repartitionImmeuble.Rows)
        {
            ...
            _ = Convert.ToInt32(row["valeur"].ToString());
    ...
    private void SaveRepartitionLot()
    {
        ...
        foreach (DataGridViewCell cell in row.Cells)
            if (cell.ColumnIndex != 0)
                if (!cell.ReadOnly)
                {
                    var entite = (LotRepartitionEntite)cell.Tag ?? new LotRepartitionEntite();
...
//            if (tbCoproprietaire.Text != "")
    ...
    {
        dataGridViewLots.DataSource =
            LotDescriptionController.getController().getDataGridListeLotDescription(immeuble, false);
