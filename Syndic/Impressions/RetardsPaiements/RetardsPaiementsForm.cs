...
public partial class RetardsPaiementsForm : Form
{
    ...
    private void commonValidating()
    {
        ...
        if (reportViewer1.Visible) btnList_Click(null, null);
        if (reportViewer2.Visible) btnRelance_Click(null, null);
    }
    ...
    private void btnRelance_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            foreach (DataGridViewRow rowGrid in dataGridView.Rows)
                if (rowGrid.Cells["relance"].Value != null)
                    if ((bool)rowGrid.Cells["relance"].Value)
                    {
                        ...
                        else if (row["type_relance"] != null)
                            type_retard = Convertir.ToInt(row["type_relance"]);
                        if (row["duplicata"].ToString() == "1" && type_retard == 3)
                            duplicatas.Add(row["id"].ToString());
                        else
                            relances[type_retard].Add(new RelanceEntite(row["immeuble_id"].ToString(),
                                row["id"].ToString(), row["lot_id"].ToString(), type_retard));
                        relancesRetard[type_retard].Add(new RelanceEntite(row["immeuble_id"].ToString(),
                            row["id"].ToString(), row["lot_id"].ToString(), type_retard));
                        bHaveSelection = true;
                    }
            ...
            if (MessageBox.Show(
                    "Voulez vous mettre à jour les dates de Relances\r\nEt Générer les écritures correspondantes", "",
            ...
            foreach (var relance in relances)
            {
                ...
                if (copros_ids != "")
                {
                    ...
                    table = OperationController.getController()
                        .GetDataForRelance(copros_ids, dt.ToShortDateString(), type);

                    if (table != null) BaseApplication.PublipostageLettreWord(table, modele);
                }
    ...
    private void dataGridView_DoubleClick(object sender, EventArgs e)
    {
        ...
        MainForm.syndicEvent.FireChanged(this,
            new CommonEventArgs(row["ref_immeuble"].ToString(), row["numero_lot"].ToString()));
    }
