...
public class CoproprietaireController : AbstractBaseController<CoproprietaireEntite>
{
    ...
    public DataTable getListeCoproForExport(bool bQuote = false)
    {
        ...
        if (bQuote)
            cmd =
                " select concat('''',i.reference) as immeuble, concat('''',c.reference) as reference, c.nom, c.prenom, ";
        else
        ...
        cmd +=
            " INNER JOIN(SELECT groupe, code, iparam_1 FROM  parametres WHERE (groupe = 'CIVILITE_POSTE')) p ON p.iparam_1 = c.codenvoi ";
    ...
    public DataTable CoproprietaireImmeubleDescriptionByLot(string immeuble_id, string numlot, bool bByNom = false,
        string limit = "")
    {
        var numero_lot = -1;
        var cmd =
        ...
        if (numlot != "" && numero_lot != -1) parameters.Add(new NpgsqlParameter("@numero_lot", numero_lot));
        Console.WriteLine(cmd);
        return getResultSQL(cmd, parameters);
    }


    public DataTable CoproprietaireImmeubleDescriptionEtiquettes(string immeuble_id, bool bByNom = false,
        string limit = "")
    {
        var cmd =
            "SELECT p.code, c.nom, c.reference, c.prenom, replace(c.adresse, '\r\n',' ') as adresse , c.codepostal, concat(c.ville, '\n', c.pays) as ville, c.id AS copro_id, d.immeuble_id, i.reference AS imm_reference, i.nom AS imm_nom, ";
        cmd += " i.rue AS imm_rue, i.ville AS imm_ville, i.codepostal AS imm_codepostal, r.valeur, d.numero_lot";
        cmd += $" FROM {getSchema()}.lot_description d  ";
        cmd += $" INNER JOIN {getSchema()}.coproprietaire c ON d.coproprietaire_id = c.id ";
        cmd +=
    ...
    public DataTable HeaderConvocationWord(string immeuble_id, NpgsqlParameter[] static_parameters = null)
    {
        ...
        cmd +=
            " i.rue as AdresseImmeuble, i.ville as VilleImmeuble, i.codepostal as CodePostalImmeuble, '' as NumeroLot, ir.valeur as valeur \n";
    ...
    public DataTable CoproprietaireImmeubleDescriptionWord(string immeuble_id,
        NpgsqlParameter[] static_parameters = null)
    {
        var schema = getSchema();
        var cmd = " Select ";
        cmd +=
            "p.code as Civilite, concat(c.nom , ' ', prenom) as Coproprietaire, c.reference as ReferenceCoproprietaire, \n";
        cmd +=
            "c.adresse as AdresseCoproprietaire, c.ville as VilleCoproprietaire, c.codepostal as CodePostalCoproprietaire,\n";
        cmd += " i.reference as ReferenceImmeuble, i.nom as NomImmeuble,\n";
        cmd +=
            " i.rue as AdresseImmeuble, i.ville as VilleImmeuble, i.codepostal as CodePostalImmeuble, l.numero_lot as NumeroLot, r.valeur as valeur \n";
