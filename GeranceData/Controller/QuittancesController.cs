...
namespace GeranceData.Controller
{
    public class QuittancesController : AbstractBaseController<QuittanceEntite>
    {
        ...
        {
            var dt = new DateTime(dtQuittance.Year, dtQuittance.Month, 1);
            var parameters = new List<NpgsqlParameter> 
            { 
                new NpgsqlParameter("@locataire_id", locataire.id),
                new NpgsqlParameter("@date_quittance", dt),
            };
            var cmd = " where locataire_id = @locataire_id and date_quittance = @date_quittance ";
            var quittance = getEntite(cmd, parameters);
        ...
        {
            decimal charges = 0;
            var cmd = "select sum(montant_charge) as charges";
        ...
        {
            var dtDeb = new DateTime(dt.Year, dt.Month, 1);
            var dtFin = dtDeb.AddMonths(1).AddDays(-1);

            var cmd = "select q.id, date_quittance, montant_quittance, l.reference as ref_locataire, trim(concat(pa2.code, ' ', l.nom, ' ', l.prenom)) as locataire, ";
            cmd += " p.reference as ref_proprio, trim(concat(pa.code, ' ', p.nom, ' ', p.prenom)) as proprietaire ";
            cmd += $"from {getSchemaTable()} q";
            cmd += $" join {getSchema()}.locataire l on l.id = locataire_id";
            cmd += $" join {getSchema()}.proprietaire p on p.id = proprietaire_id";
        ...
        public DataTable getDetailAppelDeLoyerEntree(string quittance_id)
        {
            ...
            cmd += $" join {getSchema()}.biens b on (b.id = q.bien_id) ";
        ...
        public DataTable getDetailQuittanceForImpression(string quittance_id)
        {
            ...
            var cmd = "select q.montant_loyer, q.montant_augmentation, q.valeur_taxe, q.montant_charge as montant_charges, q.frais_bail as frais_bail, q.etat_lieux  as etat_lieux, 0 as reste_du, q.honoraire_locataire as honoraires_locataire, ";
        ...
        public static bool DeleteQuittance(QuittanceEntite quittance)
        {
            ...
            var locataire = quittance.Locataire;
            try
            {
                if (locataire == null)
