...
//            DefaultOrder = ORDER;
            ...
            var cmd =
                "select b.id, b.reference, b.nom as nom_immeuble, numero_lot, trim(concat ( pa.code, ' ', p.nom, ' ', p.prenom)) as proprietaire, l.reference as ref_locataire, ";
            ...
            cmd +=
                " left join (SELECT groupe, code, iparam_1 FROM  parametres WHERE (groupe = 'CIVILITE')) pa on pa.iparam_1 = p.civilite";
            cmd +=
                " left join (SELECT groupe, code, iparam_1 FROM  parametres WHERE (groupe = 'CIVILITE')) pa2 on pa2.iparam_1 = l.civilite";
            cmd += " order by " + ORDER;
        ...
        {
            var cmd =
                "select b.reference as ref_immeuble, numero_lot, l.reference as ref_locataire, b.montant_loyer, b.montant_charges, valeur_taxe, montant_divers1, montant_divers2, montant_divers3";
            cmd += $" from {getSchemaTable()} b";
        ...
        public DataTable getListeAppelsLoyer(int iMonth = -1, string refLocataire = "", string nomLocataire = "",
            string refImmeuble = "", string nomImmeuble = "", int typeLoyer = 0, int garantie = 0, bool bLoyer = true)
        {
            var cmd =
                "select b.id, b.reference as ref_immeuble, l.reference as ref_locataire, trim(concat(pa2.code, ' ', l.nom, ' ', l.prenom)) as locataire, ";
            cmd +=
                " trim(concat(pa.code, ' ',p.nom, ' ', p.prenom)) as proprietaire, b.nom, b.montant_loyer, b.montant_charges, b.montant_augmentation";
            cmd += $" from {getSchemaTable()} b";
            cmd += $" join {getSchema()}.locataire l on l.id = b.locataire_id";
...
//            cmd += " order by b.reference";
        ...
        public DataTable getDetailAppelDeLoyer(string bien_id, DateTime dtDeb)
        {
            var cmd = "select ";
            cmd +=
                "case when coalesce (comptable_id, '') != '' then  concat(c.nom, ' ', c.prenom) else trim(concat(pa.code, ' ', l.nom, ' ', l.prenom)) end as nom_loca, ";
            cmd += " case when coalesce (comptable_id, '') != '' then c.adresse else l.adresse end as adresse, ";
            cmd +=
                " case when coalesce (comptable_id, '') != '' then c.codepostal else l.codepostal end as codepostal, ";
            ...
            cmd +=
                " case when date_quittance >= @dtDeb then l.total_du- b.montant_du else l.total_du end as reste_du, ";
            ...
            cmd +=
                " montant_loyer+ montant_augmentation+ valeur_taxe+ montant_charges+ frais_bail + honoraires_locataire + montant_divers1 + montant_divers2+montant_divers3 + montant_divers4+ montant_divers5 ) as total_du,";
...
//            cmd += " concat(l.nom, ' ', l.prenom) as nom_loca, l.adresse, l.codepostal, l.ville, l.reference, b.adresse as imm_adress, b.ville as imm_ville, b.codepostal as imm_cp";
            ...
            cmd +=
                " left join (SELECT groupe, code, iparam_1 FROM  parametres WHERE (groupe = 'CIVILITE')) pa on pa.iparam_1 = l.civilite";
        ...
        public DataTable getDetailQuittance(string bien_id)
        {
            var cmd = "select ";
            cmd +=
                " montant_loyer, montant_augmentation, valeur_taxe, montant_charges, frais_bail, l.total_du as reste_du, honoraires_locataire,";
            cmd +=
                " (montant_loyer+ montant_augmentation+ valeur_taxe+ montant_charges+ frais_bail + montant_divers1 + montant_divers2+montant_divers3 + montant_divers4+ montant_divers5 + honoraires_locataire) as total_du,";
            cmd += " montant_divers1, montant_divers2, montant_divers3, montant_divers4, montant_divers5,";
            cmd += " divers1, divers2, divers3, divers4, divers5, ";
//            cmd += " concat(l.nom, ' ', l.prenom) as nom_loca, l.adresse, l.codepostal, l.ville, l.reference, b.adresse as imm_adress, b.ville as imm_ville, b.codepostal as imm_cp";
            cmd +=
                " trim(concat(pa.code, ' ', l.nom, ' ', l.prenom)) as nom_loca, l.adresse, l.codepostal, l.ville, l.reference, b.adresse as imm_adress, b.ville as imm_ville, b.codepostal as imm_cp";
        ...
        public DataTable getDetailQuittanceFromQuittance(string bien_id, DateTime dtDeb)
        {
            ...
            var cmd =
                "select q.montant_loyer, q.montant_augmentation, q.valeur_taxe, q.montant_charge as montant_charges, q.frais_bail, 0 as reste_du, q.honoraire_locataire as honoraires_locataire,";
            cmd +=
                " (q.montant_loyer+ q.montant_augmentation+ q.valeur_taxe+ q.montant_charge+ q.frais_bail + q.montant_divers1 + q.montant_divers2+q.montant_divers3 + q.montant_divers4+ q.montant_divers5+q.honoraire_locataire ) as total_du,";
            ...
            cmd += " , b.periodicite_loyer";
            cmd += String.Format(
                " from {0}.quittances q join {0}.biens b on q.bien_id = b.id join {0}.locataire l on l.id = q.locataire_id ",
                getSchema());
            cmd +=
                " left join (SELECT groupe, code, iparam_1 FROM  parametres WHERE (groupe = 'CIVILITE')) pa on pa.iparam_1 = l.civilite";
...
//            Console.WriteLine(cmd);
        ...
        public DataTable FindElement(string bien, string proprio, string locataire, string nomBien, String nomProprio,
            String nomLoca)
        {
            bien += "%";
            proprio += "%";
            ...
            var cmd =
                "Select b.id as b_id, b.reference as bien, b.nom, p.id as p_id, p.reference as proprio , trim(concat(pa.code, ' ', p.nom, ' ', p.prenom)) as nom_proprietaire, ";
            cmd +=
                " l.id as l_id, l.reference as locataire, trim(concat(pa2.code, ' ', l.nom, ' ', l.prenom)) as nom_locataire ";
            cmd += $" from {getSchemaTable()} b ";
            ...
            cmd +=
                " left join (SELECT groupe, code, iparam_1 FROM  parametres WHERE (groupe = 'CIVILITE')) pa on pa.iparam_1 = p.civilite";
            cmd +=
                " left join (SELECT groupe, code, iparam_1 FROM  parametres WHERE (groupe = 'CIVILITE')) pa2 on pa.iparam_1 = l.civilite";
