...
namespace GeranceData.Controller
{
    public class LocataireController : AbstractBaseController<LocataireEntite>
    {
        ...
        public override DataTable GetFindList(string filter)
        {
            ...
            string cmd = String.Format("Select id, reference, trim(concat(pa.code, ' ', nom)) as nom from {0} l ", getSchemaTable());
        ...
        {
            string cmd = "select b.id, numero_lot as reference, trim(concat(pa.code, ' ', l.prenom, ' ', l.nom )) as nom ";
        ...
        {
            string cmd = "select trim(concat(pa.code, ' ', l.nom, ' ', l.prenom)) as nom, l.reference, l.adresse, concat(l.codepostal, ' ', l.ville ) as ville, b.date_entree ";
        ...
        public DataTable getListeRetardPaiements(decimal seuil, string ref_locataire = "")
        {
            ...
            string cmd = "select l.id, l.reference, ";
            ...
            cmd += string.Format(" left join {0}.comptable ct on ct.id = l.comptable_id", getSchema());
            cmd += String.Format(" join {0}.biens b on b.locataire_id = l.id and b.montant_loyer >0", getSchema());
//            cmd += string.Format(" join (select max(date_reglement) as date_reglement, locataire_id from {0}.reglements group by locataire_id) r on r.locataire_id = l.id", getSchema());
            cmd += string.Format(" left join (select max(date_reglement) as date_reglement, locataire_id from {0}.reglements group by locataire_id) r on r.locataire_id = l.id", getSchema());
            cmd += " left join (SELECT groupe, code, iparam_1 FROM  parametres WHERE (groupe = 'CIVILITE')) pa on pa.iparam_1 = l.civilite";
...
//                cmd += " and l.reference = @ref_locataire";
        ...
        {
            string cmd = "select l.reference, trim(concat(pa.code, ' ', l.nom, ' ', l.prenom))as nom, date_ecriture, libelle, debit, credit ";
            cmd += " from ( ";
