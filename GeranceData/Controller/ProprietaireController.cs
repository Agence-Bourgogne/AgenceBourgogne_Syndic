...
namespace GeranceData.Controller
{
    public class ProprietaireController : AbstractBaseController<ProprietaireEntite>
    {
        ...
        public override DataTable GetFindList(string filter)
        {
            ...
            var cmd = $"Select id, reference, trim(concat(pa.code, ' ', nom)) as nom from {getSchemaTable()} p ";
        ...
        public DataTable GetConfigList()
        {
            ...
            var cmd =
                $"select id, reference, trim(concat(pa.code, ' ', nom)) as nom, prenom, adresse, codepostal, ville,  rib, banque, statut from {getSchemaTable()} p";
            cmd += " left join (SELECT groupe, code, iparam_1 FROM  parametres WHERE (groupe = 'CIVILITE')) pa on pa.iparam_1 = p.civilite";
            cmd += $" order by {DefaultOrder}";
        ...
        protected override void setListSelectCommand()
        {
            ...
            var cmd =
                $"select id, reference, trim(concat(pa.code, ' ', nom)) as nom, prenom, adresse, codepostal, ville,  rib, banque from {getSchemaTable()} p";
            cmd += " left join (SELECT groupe, code, iparam_1 FROM  parametres WHERE (groupe = 'CIVILITE')) pa on pa.iparam_1 = p.civilite";
            cmd += $" where statut = 1 order by {DefaultOrder}";
        ...
        public DataTable getHdrFiscalProprietaire(string proprietaire_id)
        {
            ...
            cmd += $" from {getSchemaTable()} p ";
            cmd += $" join {getSchema()}.biens b on p.id = b.proprietaire_id";
        ...
        public DataTable getListePaiementsLoyers(int paiement_type , DateTime dtDeb, DateTime dtFin)
        {
            ...
            cmd += $" from {getSchemaTable()} p ";
            cmd +=
                $" join (select proprietaire_id from {getSchema()}.reglements where date_reglement >= @dtDeb and date_reglement <= @dtFin group by 1) r";
...
//            cmd += " group by 1, 2, 3, 4, 5";
        ...
        public DataTable getImpressionListePaiementsLoyers(int paiement_type, DateTime dtDeb, DateTime dtFin)
        {
            ...
            cmd +=
                $" join (select proprietaire_id,sum(debit)  + sum(honoraire_locataire) + sum(etat_lieux) as debit, sum(credit)as credit   from {getSchema()}.reglements where date_reglement >= @dtDeb and date_reglement <= @dtFin and statut <> @statut_del group by 1 ) r";
            cmd += " on r.proprietaire_id = p.id";
