...
namespace GeranceData.Controller
{
    public class FacturesController : AbstractBaseController<FactureEntite>
    {
        ...
        {
            string cmd = "select f.id, l.reference as ref_locataire,  concat(l.nom, ' ', l.prenom) as locataire, b.reference as ref_immeuble, b.nom as immeuble, f.date_facture, f.libelle, abs(f.debit) as debit,abs(f.credit) as credit";
        ...
        public DataTable getDeductionProprio(DateTime dtDeb, DateTime dtFin, string proprietaire_id = "")
        {
            ...
            cmd += " order by p.reference, date_facture ";
...
//            cmd += " and debit = @montant";
  ...
  //              Console.WriteLine(c);
        ...
        {
            string cmd = "select ";
            cmd += " n.declaration, n.nom, sum(f.debit) - sum(f.credit) as montant ";
            cmd += String.Format(" from {0} f ", getSchemaTable());
